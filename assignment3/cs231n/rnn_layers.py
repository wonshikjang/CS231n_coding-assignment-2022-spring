...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      if i == 0:
        ...
        h_i = h[:,i-1,:]
      x_i = x[:,i,:].reshape(N,D)
      h[:,i,:], caches[i] = rnn_step_forward(x_i, h_i, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      dh_i = dh[:,i,:].reshape(N,H) + dnext_h
      if i == 0 :
        dx[:,i,:], dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh_i, caches[i])
      else:
        dx[:,i,:], dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_i, caches[i])
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
...
def word_embedding_forward(x, W):
    ...
    #sentence consist of words, each word is represented by word vector
    #x[i,j] means the index of word in a sentence
    out = W[x,:]
    cache = (x,W)
