...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #(1)
        word_vector = self.embedding(captions)
        word_pos = self.positional_encoding(word_vector)
        features_proj = self.visual_projection(features).unsqueeze(1)

        #(2)
        tgt_mask = torch.tril(torch.ones(T, T,device=word_pos.device,
                                        dtype=word_pos.dtype))

        #(3)
        features = self.transformer(tgt=word_pos,memory=features_proj,
                                    tgt_mask=tgt_mask)
