...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pos = i.unsqueeze(1) * 10000**(-j/embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        K_t = K.transpose(2,3)
        scale = torch.matmul(Q,K_t) / math.sqrt(E/H)
        if attn_mask is not None:
          scale = scale.masked_fill(attn_mask==0, -math.inf)

        softmax = nn.Softmax(dim= -1)
        score = softmax(scale)
        score = self.attn_drop(score)
        attn_weight_head= torch.matmul(score,V)
        attn_weight = attn_weight_head.transpose(1,2).contiguous().view(N,S,E)
        output = self.proj(attn_weight)
